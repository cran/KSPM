## ----setup, include = FALSE----------------------------------------------
# Sys.setenv(hydat_eval = TRUE)
LOCAL <- nzchar(Sys.getenv("hydat_eval"))
print(LOCAL)
...
## ---- eval = FALSE-------------------------------------------------------
#  # NOT RUN
#  # limitCores option should be chosen dependently of computer characteristics. see ?detectCores function
#  csm.fit4 <- kspm(response = "Ratings", linear = NULL, kernel = ~Kernel(~ Sentiment + Views + Likes + Dislikes + Comments + Aggregate.Followers, kernel.function = "gaussian"), data = csm, level = 0, control = kspmControl(parallel = TRUE, limitCores = 5))
